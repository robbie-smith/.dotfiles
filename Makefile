...
deploy:
	...
	@AWS_DEV_ACCOUNT=$(AWS_DEV_ACCOUNT) AWS_DEV_REGION=$(AWS_DEV_REGION) brazil-build cdk list | { \
			...
			DEV_SERVICE_STACK=$$(echo "$$CDK_LIST_OUTPUT" | grep -o '[^ ]*-personal' | grep -v 'BONESBootstrap' | head -n 1); \
			if [ -z "$$DEV_SERVICE_STACK" ]; then \
					echo "Error: No service stack with '-dev' suffix found."; \
					exit 1; \
			fi; \
			echo "Deploying service stack: $$DEV_SERVICE_STACK"; \
			AWS_DEV_ACCOUNT=$(AWS_DEV_ACCOUNT) AWS_DEV_REGION=$(AWS_DEV_REGION) brazil-build cdk deploy $$DEV_SERVICE_STACK --require-approval never; \
	}
